...
class Client {
  constructor(accountid, opts) {
    ...
    this.refreshSTSTokenInterval = opts.refreshSTSTokenInterval !== undefined
      ? opts.refreshSTSTokenInterval
      : 300000; // 默认 5 分钟
    ...
    if (this.securityToken && !this.refreshSTSToken) {
      debug('Warning: securityToken provided without refreshSTSToken callback. ' +
            'Token will not be automatically refreshed when it expires.');
    }
  ...
  async _doRefreshSTS(now) {
    try {
      const credentials = await this.refreshSTSToken();
      ...
      // 刷新失败时记录警告，继续使用旧凭证
      debug('STS token refresh failed: %s. Will continue with current credentials.', err.message);
      // 更新刷新时间，避免频繁重试
      this.stsTokenFreshTime = now;
      // 不抛出错误，让请求继续使用当前凭证
    }
  ...
  async request(method, resource, type, requestBody, attentions = [], opts = {}) {
    await this.refreshSTS();  // 请求前刷新 STS Token
    ...
    const response = await httpx.request(url, Object.assign(opts, requestOpts));
  ...
  post(resource, type, body, attentions = [], opts = {}) {
    return this.request('POST', resource, type, body, attentions, opts);
  }

  delete(resource, type, body, attentions = [], opts = {}) {
  ...
  // Message
  sendMessage(queueName, params, timeout = 30) {
    const url = `/queues/${queueName}/messages`;
    const body = toXMLBuffer('Message', params);
    return this.post(url, 'Message', body, {timeout: timeout});
  }

  async batchSendMessage(queueName, params, timeout = 30) {
    const url = `/queues/${queueName}/messages`;
    const subType = 'Message';
    const body = toXMLBuffer('Messages', params, subType);
    var response = await this.post(url, 'Messages', body, {timeout: timeout});
    response.body = response.body[subType];
    return response;
  }
